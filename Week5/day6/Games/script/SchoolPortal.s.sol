...
import {IERC20} from "../class/IERC20";
...
contract SchoolPortal{
    ...
    function getStaffById(uint8 _id) external view {
    ...
    function updateStaff(uint _id, string _name, address _acct) returns (bytes32){
        bytes32 isFound;
        for(uint8 i; i<staff.length; i++){
            if(staff[i].id == _id){
                isFound = "Staff Found";
    ...
    //STUDENT
    function CreateStudent(string memory _name, uint16 _level, string _email) external{
        ...
        bool success = paymentToken.transferFrom(address(this), fees);
    ...
    function studentById(uint8 _id) external view {
    ...
    function updateStudent(uint8 _id, string _name, string _email) external returns(bytes32)  {
        bytes32 isFound;
        for(uint8 i; i<students.length; i++){
            if(students[i].id == _id){
                isFound = "Stud Found";
    ...
    function deleteStudent(uinit8 _id){
        for(uinit8 i; i<students.length; i++){
            if(students[i].id == _id){
